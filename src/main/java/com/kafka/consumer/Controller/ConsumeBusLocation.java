...
public class ConsumeBusLocation {
    ...
    public void consumeLastStation(ConsumerRecord<String, BusTracker> record) {
        ...
        Mono<Void> process = stationService.saveStation(lastLocation.getBusId(),
                        lastLocation.getLastStationLocation().getX(),
                        lastLocation.getLastStationLocation().getY())
                .then(stationService.saveLastLocation(lastLocation.getBusId(),
                        lastLocation.getLastStationLocation().getX(),
                        lastLocation.getLastStationLocation().getY()))
                .then(stationService.calculateSpeed(lastLocation.getBusId(), lastLocation.getLastStationLocation().getX(), lastLocation.getLastStationLocation().getY())
                        .doOnNext(speed -> {
